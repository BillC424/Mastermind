class ComputerPlayer
  ...
  private
    ...
    def make_selections
      ...
      if @correct_computer_choices == %w(incorrect incorrect incorrect incorrect) && @correct_computer_color_choices == []
      ...
      human.secret_colors.each_with_index do |secret_color, index|
        if secret_color == @computer_choices[index]
          ...
          human.secret_colors.each do |secret_color|
            ...
            @correct_color += 1
            @correct_computer_color_choices.push(secret_color) unless @correct_computer_color_choices.include?(secret_color)
          end
        end
      end
      human.secret_colors.each_with_index do |secret_color, index|
